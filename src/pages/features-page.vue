    ...
    <section class="features-stack" aria-label="Features inhoud">
      <section class="page-section intro-section" aria-label="Features intro">
        <div class="section-container intro-content" v-scroll-reveal>
          <p class="intro-copy">
            Noek biedt jouw team een white-label platform waarmee families samen herinneringen kunnen delen en koesteren.
            Persoonlijk, voelbaar, en ontworpen voor elk moment van het rouwproces.
          </p>
        ...
        <div class="section-container previews-shell">
          <div class="previews-desktop" v-scroll-reveal="{ delay: 30 }">
            <figure v-for="(screen, index) in screens" :key="`desktop-${index}`" class="preview-phone">
              <img v-if="screen" :src="screen" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 70vw, 300px" :alt="`Noek interface – pagina ${index + 1}: herdenkingsruimte interactie`" width="300" height="450" loading="lazy" decoding="async" />
            ...
            <div class="carousel-window">
              <div class="carousel-track" :style="carouselStyle">
                <figure v-for="(screen, index) in screens" :key="`mobile-${index}`" class="preview-phone carousel-slide">
                  <img v-if="screen" :src="screen" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 70vw, 300px" :alt="`Noek app – pagina ${index + 1}: herdenkingsruimte op mobiel`" width="300" height="450" loading="lazy" decoding="async" />
                  <span v-else class="preview-placeholder">Preview {{ index + 1 }}</span>
                </figure>
              </div>
            </div>
...
useHead(
  createPageHead({
    title: 'Mogelijkheden: digitale herdenkingsruimte voor jouw team',
    description:
      'Ontdek hoe Noek jouw team helpt families samen herinneringen te bewaren. White-label platform met troostende sfeer.',
    path: '/features',
...
const featureItems = computed(() => [
  ...
  {
    title: 'Troostende sfeer',
    description:
      'Subtiele geluiden en licht creëren een warme, voelbare omgeving die mensen samenbrengt in een moeilijk moment.',
    icon: assetPaths.icons.featureAtmosphere,
    fallback: '◍'
  },
  {
    title: 'Flexibele hosting',
    description: 'Host op je eigen domein of gebruik één van onze subdomeinen—jij beslist.',
  ...
  },
  {
    ...
    description:
      'Elke plek wordt precies zo ingevuld als de overledene verdient—met foto\'s, verhalen en momenten die tellen.',
